...
export function calculateFarmerSeasonAccount(input: FarmerSeasonAccountInput): FarmerSeasonAccountSummary {
  const loanTotal = roundMoney(input.loans.reduce((sum, loan) => sum + toNumber(loan.amount), 0))
  const repaymentTotal = roundMoney(
    input.repayments.reduce((sum, repayment) => {
...
export function calculateSeasonReport(input: SeasonReportInput): SeasonReportSummary {
  const totalInvested = roundMoney(input.loans.reduce((sum, loan) => sum + toNumber(loan.amount), 0))
  const repaymentValue = roundMoney(
    input.repayments.reduce((sum, repayment) => sum + (toNumber(repayment.bags) * toNumber(repayment.price_per_bag)), 0),
  )
  const repaymentBags = input.repayments.reduce((sum, repayment) => sum + toNumber(repayment.bags), 0)
  const paddyBagsReceived = input.stockMovements
  ...
  const paddyBagsMilled = input.millingBatches.reduce((sum, batch) => sum + toNumber(batch.paddy_bags), 0)
  const milledBagsProduced = input.millingBatches.reduce((sum, batch) => sum + toNumber(batch.milled_bags), 0)
  const milledBagsDispatched = input.dispatches.reduce((sum, dispatch) => sum + toNumber(dispatch.bags), 0)
  const expensesTotal = roundMoney(input.expenses.reduce((sum, expense) => sum + toNumber(expense.amount), 0))
  const revenueTotal = roundMoney(input.dispatches.reduce((sum, dispatch) => sum + toNumber(dispatch.sale_amount), 0))
  ...
  return {
    ...
    revenueTotal,
    profit,
    warehouseBalances: calculateWarehouseBalances(input.stockMovements),
  }
}
